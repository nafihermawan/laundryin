...
export function BottomNav({ homeHref }: { homeHref: string }) {
  ...
  const isHistoryList = pathname === "/kasir/riwayat";

  if (pathname === homeHref) {
    return (
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-zinc-200 bg-white">
        <div className="mx-auto w-full max-w-6xl px-4">
          <nav className="flex h-14 items-center justify-end gap-2 text-sm">
            <Link
              href="/kasir/riwayat"
              prefetch={false}
              className={`inline-flex h-10 w-10 items-center justify-center rounded-xl border border-zinc-200 bg-white transition hover:border-zinc-300 hover:bg-zinc-50 ${
                isHistoryList ? "text-sky-700" : "text-zinc-700 hover:text-zinc-900"
              }`}
      ...
      <div className="mx-auto w-full max-w-6xl px-4">
        <nav className="relative flex h-14 items-center justify-center text-sm">
          ...
          <Link
            href="/kasir/riwayat"
            prefetch={false}
            className={`absolute right-0 inline-flex h-10 w-10 items-center justify-center rounded-xl border border-zinc-200 bg-white transition hover:border-zinc-300 hover:bg-zinc-50 ${
              isHistoryList ? "text-sky-700" : "text-zinc-700 hover:text-zinc-900"
            }`}
            aria-label="Riwayat Transaksi"
            title="Riwayat Transaksi"
